...
if len(features_pdf) > 10:
    try:
        ...
        if len(X) == 0:
            raise Exception("LSTM sequence generation returned zero samples")
        ...
        if model_used == "prophet":
            ...
            model_used = "lstm"
        print(f"LSTM forecast success: {lstm_next}")
    except Exception as e:
...
# Create new single forecast record for today's run
new_forecast_output = pd.DataFrame([{
    "forecast_date": str((max_date + pd.Timedelta(days=1)).date()),
    "ticker": ticker_val,
...
new_forecast_spark_df = spark.createDataFrame(new_forecast_output)

# Accumulate forecast history across runs
forecast_exists = False
try:
    existing_forecast_spark = spark.read.parquet(forecast_path)
    if not existing_forecast_spark.rdd.isEmpty():
        forecast_exists = True
except Exception:
    forecast_exists = False

if forecast_exists:
    final_forecast_spark_df = (
        existing_forecast_spark
        .unionByName(new_forecast_spark_df)
        .dropDuplicates(["forecast_date", "ticker"])
        .sort("forecast_date")
    )
else:
    final_forecast_spark_df = new_forecast_spark_df

final_forecast_spark_df.write.mode("overwrite").parquet(forecast_path)
write_single_csv(final_forecast_spark_df, forecast_csv_path)

# Create new single metrics record for today's run
new_metrics_output = pd.DataFrame([{
    "ticker": ticker_val,
    "silver_rows": int(base_rows),
    "feature_rows": int(len(features_pdf)),
    "max_training_date": str(max_date.date()),
    "latest_close": float(latest_close),
    "prophet_status": prophet_status,
    "lstm_status": lstm_status,
    "prophet_error": prophet_error[:500],
    "lstm_error": lstm_error[:500],
    "model_used": model_used,
    "run_timestamp": str(datetime.now())
}])

new_metrics_spark_df = spark.createDataFrame(new_metrics_output)

# Accumulate metrics history across runs
metrics_exists = False
try:
    existing_metrics_spark = spark.read.parquet(metrics_path)
    if not existing_metrics_spark.rdd.isEmpty():
        metrics_exists = True
except Exception:
    metrics_exists = False

if metrics_exists:
    final_metrics_spark_df = (
        existing_metrics_spark
        .unionByName(new_metrics_spark_df)
        .dropDuplicates(["max_training_date", "ticker"])
        .sort("max_training_date")
    )
else:
    final_metrics_spark_df = new_metrics_spark_df

final_metrics_spark_df.write.mode("overwrite").parquet(metrics_path)
write_single_csv(final_metrics_spark_df, metrics_csv_path)

if max_loaded_date:
    wm_text = f'{{"stock_name":"hdfc","last_loaded_date":"{max_loaded_date}"}}'
    dbutils.fs.put(watermark_path, wm_text, True)

print("Gold parquet and CSV written successfully")
print(f"Final model_used: {model_used}")
print(f"Prophet status: {prophet_status}")
print(f"LSTM status: {lstm_status}")
dbutils.notebook.exit("gold_done")"""
